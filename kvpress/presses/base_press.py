...
class BasePress:
    ...
    def __call__(self, model: PreTrainedModel) -> Generator:
        ...
        hooks = []
        try:
            # Resolve base language model across architectures
            if hasattr(model, "model"):
                base_model = model.model
                language_model = base_model.language_model if hasattr(base_model, "language_model") else base_model
            ...
            for i, layer in enumerate(language_model.layers):
                # Determine attention module name per architecture
                ...
                # Skip Gemma3 sliding window layers
                if isinstance(model, Gemma3ForConditionalGeneration) and hasattr(layer, "self_attn") and layer.self_attn.is_sliding:
                    continue

                # Ensure rotary embedding reference and layer index
                if hasattr(language_model, "rotary_emb"):
                    attn.rotary_emb = language_model.rotary_emb
                setattr(attn, "layer_idx", i)
